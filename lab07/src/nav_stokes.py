...
def error(zeta, psi):
    result = 0.0
    for i in range(1, N_X):
        result += psi[i + 1][j2] + psi[i - 1][j2] + psi[i][j2 + 1] + psi[i][j2 - 1] - 4 * psi[i][j2] - DELTA * DELTA * \
                  zeta[i][j2]
    return result
...
def nav_stokes_numba(Qwe):
    ...
    map_generator(tmp_x, tmp_y, u, rf'Q={Qwe}, u(x,y)', f'u_{Qwe}.png')
    map_generator(tmp_x, tmp_y, v, rf'Q={Qwe}, v(x,y)', f'v_{Qwe}.png')
    contour_generator(tmp_x, tmp_y, zeta, rf'Q={Qwe}, $\zeta(x,y)$', f'zeta_{Qwe}.png')
    contour_generator(tmp_x, tmp_y, psi, rf'Q={Qwe}, $\psi(x,y)$', f'psi_{Qwe}.png')
