...
def rk4():
    params_for_chart = {
        "title": "z3. - Metoda RK4 - rozwiązanie",
    ...
    for i, time_step in enumerate(time_steps):
        ...
        for j in range(n + 1):
            ...
            k1 = param_lambda * y_n
            k2 = param_lambda * (y_n + (time_step / 2.0) * k1)
            k3 = param_lambda * (y_n + (time_step / 2.0) * k2)
            k4 = param_lambda * (y_n + time_step * k1)

            x_n = j * time_step
            x[i].append(x_n)

            y_err[i].append(y_n - euler_dok(x_n, param_lambda))
            y_n = y_n + (time_step / 6.0) * (k1 + 2.0 * k2 + 2.0 * k3 + k4)
    ...
    params_for_chart = {
        "title": "z3. - Metoda RK4 - błąd globalny",
        "legend": [r"$\it{\Delta t}=0.01$ s", r"$\it{\Delta t}=0.1$ s", r"$\it{\Delta t}=1$ s"],
        "filename": 'rk4_err.png',
    }
